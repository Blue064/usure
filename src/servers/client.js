...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[usure.one/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[usure.one/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    let serversdata = require("../../src/database/models/servers/server.js");
    const servers = await serversdata.find();
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: + servers.length+' Servers | usure.one/servers' }, status: "dnd" });
});
