...
module.exports = async (client) => {
  ...
  app.get("/robots.txt", function(req, res) {
    ...
    res.send(`Sitemap: https://usure.one/sitemap.xml`);
  });
  app.get("/sitemap.xml", async function(req, res) {
    let link = "<url><loc>https://usure.one/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
      link += "\n<url><loc>https://usure.one/bot/" + bot.botID + "</loc></url>";
    })
  ...
  app.get("/callback", passport.authenticate("discord", {
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://usure.one/user/${a.id}) The user named **site** tried to log in but could not log in because he was blocked from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    })
    req.session.destroy(() => {
      res.json({
        login: false,
        message: "You have been blocked from usure.",
        logout: true
      })
      req.logout();
    });
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://usure.one/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
...
app.post("/admin/news", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  const msg = new webhook.MessageBuilder()
    ...
    .setDescription(`<@${req.user.id}> Posted a News \n\nLink:\n[website](https://usure.one/news)`)
    .setColor('#0099ff')
    .setFooter(`Copyright © usure.one official 2021`)
  Hook.send(msg);
...
http.listen(3000, () => { console.log("[usure.one]: Website running on 3000 port.") });
...
console.log(`
      [===========================================]
                       usure.one
        https://github.com/usure-one/usure
                Developed by Mr Usure
                    Achievements =)
      [===========================================]
      `)
...
sleep(1050)
console.clear();
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: General routers loading...");
sleep(500);
...
/* Uptime System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Uptime system routers loading...");
sleep(500);
...
/* Profile System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Profile system routers loading...");
sleep(500);
...
/* Code Share System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Code Share system routers loading...");
sleep(500);
...
/* Botlist System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Botlist system routers loading...");
sleep(500);
...
/* Server List System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Serverlist system routers loading...");
sleep(500);
...
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Admin Panel system routers loading...");
sleep(500);
...
/* Bot System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[usure.one]: Bot system loading...");
