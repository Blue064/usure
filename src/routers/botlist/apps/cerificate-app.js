...
console.log("[usure.one]: Botlist/Certificate Application router loaded.");
...
app.post("/certification/apply", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.botlog).send(`User **${req.user.username}** requested a certificate for the bot named **${botdata.username}**.`)
});
