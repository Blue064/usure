...
console.log("[usure.one]: Botlist/Add Bot router loaded.");
    ...
    app.post("/addbot", global.checkAuth, async (req,res) => {
      ...
      if(rBody['coowners']) {
          if(String(rBody['coowners']).split(',').length > 3) return res.redirect("?error=true&message=You can add up to 3 CO-Owners..")
          if(String(rBody['coowners']).split(',').includes(req.user.id)) return res.redirect("?error=true&message=You cannot add yourself to CO-Owners.");
