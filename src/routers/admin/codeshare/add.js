...
console.log("[usure.one]: Admin/CodeShare/Add router loaded.");
...
app.post("/admin/addcode", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.codelog).send(new Discord.MessageEmbed()
        ...
        .setDescription(`The user named **[${req.user.username}](https://usure.one/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
        .addField("Code Link", `https://usure.one/code/${kod}`, true)
