...
console.log("[usure.one]: Admin/Maintence router loaded.");
...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`<a:dis_off:855688791434985472> usure has been switched to __Maintenance__ due to **${req.body.reason}** [||<@&861221279080120371>||]`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesusure = new dc.MessageEmbed()
        .setAuthor("usure.one", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<a:online:833375738785824788> <a:dis_on:855688790391521290> usure are **active** again!\n[Click to redirect website](https://usure.one)`)
        .setFooter("usure © All rights reserved.");
    await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
        a.edit(`~~ <a:online:833375738785824788> usure has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesusure)
    })
