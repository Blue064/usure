...
console.log("[usure.one]: Admin/Botlist/Certificate Give router loaded.");
...
app.get("/admin/certificate/give/:botID", global.checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(global.config.server.channels.botlog).send(`<:check:853262343949254696> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate.`)
    client.users.cache.get(botdata.ownerID).send(`<:check:853262343949254696> Your bot named **${bota.tag}** has been certified.`)
  });
