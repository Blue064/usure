...
console.log("[usure.one]: Admin/Botlist/Certificate Decline router loaded.");
...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:notcheck:853262343790526495> <@${botdata.ownerID}>'s bot named **${bota.tag}** has not been granted a certificate.`)
