...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[usure.one]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("[usure.one]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'usure.one | '+bots.length+' bots' }, status: "online" });
});
...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    client.channels.cache.get('859637418528735253').send(`Hey <@${newState.member.id}>, Thankyou for being active! You have got some **usure Coins** for being active!`)
    }
